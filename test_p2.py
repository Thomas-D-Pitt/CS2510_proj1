...
def get_args(argv):
    ...
    parser_init = subparsers.add_parser('clients', description='Initialize Docker clients')
    parser_init.set_defaults(func=init_client_containers)
...
def client_ip(client_id):
    return IP_BASE+str(200+client_id)
...
def init_containers():
    for i in range(1,NUM_SERVERS+1):
        cmd_str = "docker run --cap-add=NET_ADMIN -itd --name {serv} --network {net} --ip {ip} {img} {cmd} {args}".format(serv=server_name(i), net=NETWORK_NAME,
...
# Create client containers and attach them to bridge network
def init_client_containers(args):
    client_cmd = "python3 clientAuto.py"
    for i in range(1,NUM_SERVERS+1):
        cmd_str = "docker run --cap-add=NET_ADMIN -itd --name {client} --network {net} --ip {ip} {img} {cmd} {args}".format(client=client_name(i), net=NETWORK_NAME,
        img=IMAGE_NAME, ip=client_ip(i), cmd=client_cmd, args="--id {}".format(i))
        subprocess.Popen(cmd_str, shell=True)
        #subprocess.Popen(cmd_str, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    return
...
def cleanup(args):
    for i in range(1,NUM_SERVERS+1):
        ...
        cmd_str = "docker kill {serv}".format(serv=server_name(i))
        subprocess.run(cmd_str, shell=True)
    ...
    cmd_str = "docker kill cs2510_client1"
    subprocess.run(cmd_str, shell=True)

    cmd_str = "docker rm cs2510_client1"
    subprocess.run(cmd_str, shell=True)

    cmd_str = "docker kill cs2510_client2"
    subprocess.run(cmd_str, shell=True)

    cmd_str = "docker rm cs2510_client2"
    subprocess.run(cmd_str, shell=True)

    cmd_str = "docker kill cs2510_client3"
    subprocess.run(cmd_str, shell=True)

    cmd_str = "docker rm cs2510_client3"
    subprocess.run(cmd_str, shell=True)
